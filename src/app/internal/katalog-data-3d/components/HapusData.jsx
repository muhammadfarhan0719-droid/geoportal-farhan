...
const HapusData = ({ item, handleCloseDelete, getData, accessToken }) => {
    ...
    const handleDelete = async () => {
        const data3dId = item?.data_3d_id || item?.id;
        if (!data3dId) {
        ...
        try {
            const response = await fetch(`/portal/api/katalog-data-3d/delete?data_3d_id=${data3dId}`, {
                method: "DELETE",
                headers: {
            ...
            if (typeof handleCloseDelete === "function") {
                handleCloseDelete();
            }
            ...
            <Box sx={{ textAlign: "center", py: 1 }}>
                ...
                <Box sx={{ display: "flex", gap: 2, justifyContent: "center" }}>
                    <Button
                        ...
                        onClick={(e) => {
                            e.stopPropagation();
                            if (typeof handleCloseDelete === "function") handleCloseDelete();
                        }}
                        disabled={loading}
                        sx={{
                            flex: 1,
